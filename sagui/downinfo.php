...
$idunidade=unidade($_GET["ip"]);
$result=pg_exec($conn,"SELECT  p.nome,p.shellvar,up.valor from unidadeparametroslocais up,parametroslocais p where idunidade=$idunidade and up.idparametro=p.id");
$lines=pg_numrows($result);
if ($lines==0){
	echo "echo \"Sem Parametros para essa unidade\"\n";
	//ERRNOTLOC usado pelo CEES  
	echo "ERRNOTLOC=1\n";
	//	echo "exit 1\n";
	//	exit;
}
